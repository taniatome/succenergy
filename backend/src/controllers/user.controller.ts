import type { Request, Response } from 'express';

import { requireUser } from '../middleware/auth.js';
import {
  createProfileSchema,
  saveOnboardingSchema,
  updateProfileSchema,
} from '../schemas/user.schema.js';
import { accountService } from '../services/account.service.js';
import type { Caller } from '../services/caller.js';
import { onboardingService } from '../services/onboarding.service.js';
import { userService } from '../services/user.service.js';

/**
 * HTTP layer only: parse, delegate, respond.
 *
 * No SQL, no business rules. Every handler here reads as three lines because
 * anything more would be logic that belongs in a service.
 */

/** Narrows the verified request user to what the services need. */
export function callerFrom(req: Request): Caller {
  const user = requireUser(req);
  return { uid: user.uid, email: user.email };
}

/**
 * GET /v1/me
 *
 * A pure read. 404 `profile_not_found` when the uid has no row, which tells
 * the client to call POST first.
 */
export async function getMe(req: Request, res: Response): Promise<void> {
  const profile = await userService.getProfile(callerFrom(req));
  res.json({ data: profile });
}

/**
 * POST /v1/me
 *
 * First contact: creates the profile for the authenticated uid, with whatever
 * registration collected in the body. 201 on creation, 200 when the row
 * already exists, so a retry is harmless rather than a conflict.
 */
export async function createMe(req: Request, res: Response): Promise<void> {
  const input = createProfileSchema.parse(req.body);

  const { profile, created } = await userService.createProfile(
    callerFrom(req),
    input,
  );
  res.status(created ? 201 : 200).json({ data: profile });
}

/** PATCH /v1/me */
export async function updateMe(req: Request, res: Response): Promise<void> {
  const input = updateProfileSchema.parse(req.body);

  const profile = await userService.updateProfile(callerFrom(req), input);
  res.json({ data: profile });
}

/**
 * DELETE /v1/me
 *
 * Returns 200 with a count rather than 204, so the caller has confirmation of
 * how much was removed instead of an empty body it has to trust.
 */
export async function deleteMe(req: Request, res: Response): Promise<void> {
  const result = await accountService.deleteAccount(callerFrom(req));
  res.json({ data: { deleted: true, documentsDeleted: result.documentsDeleted } });
}

/** GET /v1/me/onboarding */
export async function getOnboarding(req: Request, res: Response): Promise<void> {
  const response = await onboardingService.getOnboarding(callerFrom(req));
  res.json({ data: response });
}

/** POST /v1/me/onboarding */
export async function saveOnboarding(req: Request, res: Response): Promise<void> {
  const input = saveOnboardingSchema.parse(req.body);

  const response = await onboardingService.saveOnboarding(
    callerFrom(req),
    input,
  );
  res.status(201).json({ data: response });
}
